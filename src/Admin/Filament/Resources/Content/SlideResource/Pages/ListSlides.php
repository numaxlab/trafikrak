...
class ListSlides extends BaseListRecords
{
    ...
    {
        return [
            Section::HOMEPAGE->value => Tab::make(__('testa::slide.form.section.options.homepage'))
                ->modifyQueryUsing(fn (Builder $query) => $query->where('section', Section::HOMEPAGE->value)),
            Section::BOOKSHOP->value => Tab::make(__('testa::slide.form.section.options.bookshop'))
                ->modifyQueryUsing(fn (Builder $query) => $query->where('section', Section::BOOKSHOP->value)),
            Section::EDITORIAL->value => Tab::make(__('testa::slide.form.section.options.editorial'))
                ->modifyQueryUsing(fn (Builder $query) => $query->where('section', Section::EDITORIAL->value)),
            Section::EDUCATION->value => Tab::make(__('testa::slide.form.section.options.education'))
